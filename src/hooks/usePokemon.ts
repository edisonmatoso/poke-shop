...
import { Pokemon, PokemonDetail, PokemonType } from '../types'
...
const usePokemon = ({ type }: { type: PokemonType }) => {
  ...
  const { get, loading, error } = useFetch<PokemonFetch>(`/type/${type}`)
  ...
  const handleAddCart = (pokemon: PokemonDetail) => {
    ...
    if (indexFound > -1) {
      ...
      const changedItemFound = {
        ...itemFound,
        quantity: itemFound.quantity + 1,
      }

      const newCart = Object.assign([...cart], {
        [indexFound]: changedItemFound,
      })
  ...
  const handleRemoveCart = (pokemon: PokemonDetail) => {
    ...
    if (indexFound > -1 && cart[indexFound].quantity === 1) {
      const newCart = [
        ...cart.slice(0, indexFound),
        ...cart.slice(indexFound + 1),
      ]
      setCart(newCart)
    }
